...
describe('Wave #30 — outputs set in all comment modes', () => {
  ...
  it('toActionOutputs sets comment_url when provided', () => {
    ...
    const { runAccountChecks } = require('../src/checks');
    const result = runAccountChecks(account, DEFAULT_CHECK_CONFIG);
    const outputs = toActionOutputs(result, 'https://github.com/issue/1#comment-42');
  ...
  it('horizon failure result has account_funded=false', () => {
    const result = horizonFailureResult('503 Service Unavailable', DEFAULT_CHECK_CONFIG);
    const outputs = toActionOutputs(result);
    expect(outputs.account_funded).toBe('false');
  });
});
...
describe('Wave #38 — scale: validation gate independence', () => {
  ...
  it('produces independent results for 100 contributor addresses', () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const account = {
        ...
        num_sponsored: 0,
        balances: [
          { balance: '10.0000000', asset_type: 'native' as const, buying_liabilities: '0', selling_liabilities: '0' },
          { balance: '100.0', asset_type: 'credit_alphanum4' as const, asset_code: 'USDC', asset_issuer: USDC_ISSUER, buying_liabilities: '0', selling_liabilities: '0' },
        ],
      };
  ...
  it('mixed funded/unfunded results are never shared across calls', () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      if (i % 2 === 0) {
        const account = {
          id: addr, account_id: addr, sequence: '1', subentry_count: 1,
          num_sponsoring: 0, num_sponsored: 0,
          balances: [
            { balance: '5.0', asset_type: 'native' as const, buying_liabilities: '0', selling_liabilities: '0' },
            { balance: '100.0', asset_type: 'credit_alphanum4' as const, asset_code: 'USDC', asset_issuer: USDC_ISSUER, buying_liabilities: '0', selling_liabilities: '0' },
          ],
        };
